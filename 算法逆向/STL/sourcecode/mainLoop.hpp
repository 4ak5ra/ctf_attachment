...
public:
    ...
    std::map<int, Handler*> msg_queue;//储存cmd
    ...
    void looper_start() {
        ...
        while (true) {
            ...
            if (bytesRead >= sizeof(CMD)) {
                CMD* cmd = reinterpret_cast<CMD*>(msg); //按命令体来解析
                if(cmd->cnt > 0x30)
                {
                ...
                unsigned int actualSize = sizeof(int)*3 + 1; //size of CMD

                char* cmdData = cmdStruct->data;  //取出CfgCMD信息
                for (unsigned int i = 0; i < cmdStruct->cnt; ++i) {
                    CfgCMD* cfgCmd = parseTLVCfgCMD(cmdData); //解析一个配置命令体
                    if(cfgCmd == nullptr)
                    {
                        std::cerr << "Invalud TLV cfg!" << std::endl;
                        break;
                    }
                    cfgcmd_queue.push_back(cfgCmd); //临时储存解析后的对象
                ...
                if (bytesRead == actualSize) {
                    handleCMD(msg); //根据handleCMD来处理命令
                    // here delete all cfgcmd，删除临时解析后的对象
                    for (auto& cmd : cfgcmd_queue) {
                        delete [] cmd->content.config_content;
                        delete [] cmd->name.config_name;
                        delete cmd; // 这会调用CfgCMD的析构函数，进而释放Config的内存 不懂
                    }
